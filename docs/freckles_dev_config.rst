...
This is how to kick of the process:
...
Also note the ``vars.project_repo`` and ``vars.project_dir`` variables used in this configuration. Those come in hand if you, for example, want to use a different directory than ``~/projects/freckles`` as destination for the *freckles* source checkout. Or you have cloned the repo and want to use your clone.
...
...in detail works (check `here <XXX>`_ if you are interested). Suffice to say, this is using the same templating language and extensions ansible_ uses. *freckles*, so far, supports 2 variable 'sources':
...
Because *freckles* overlays configuration values on subsequent config files, while keeping non-overlayed ones around, it is fairly easy to customize the base ``conda.yml`` config:
...
Easy. Well, not as easy as conda, since we need to do a bit more legwork, but the config is still fairly easy to understand:
